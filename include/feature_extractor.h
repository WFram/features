...
#ifndef WF_FEATURE_EXTRACTOR_FEATURE_EXTRACTOR_H
#define WF_FEATURE_EXTRACTOR_FEATURE_EXTRACTOR_H
...
class ORBFeatureExtractor {
 public:
  ...
  float ICAngle(const cv::Mat &image, cv::Point2f pt, const std::vector<int> &u_max) const;

  void computeOrientation(const cv::Mat &image, Keypoints &keypoints, const std::vector<int> &u_max) const;

  void computeKeypointsOctTree(std::vector<Keypoints> &all_keypoints) const;

  class ExtractorNode {
   public:
    explicit ExtractorNode() : no_more_(false) {}

    friend void ORBFeatureExtractor::distributeOctTree(const Keypoints &distributed_keypoints,
                                                       Keypoints &result_keypoints, const int &min_x, const int &max_x,
                                                       const int &min_y, const int &max_y, const size_t &N) const;
   ...
   private:
    ...
    cv::Point2i u_l_, u_r_, b_l_, b_r_;
    std::list<ExtractorNode>::iterator node_iterator_;
    bool no_more_;
  };
 ...
 private:
  ...
  const int half_patch_size_ = 15;
  const int edge_threshold_ = 19;

  const int init_fast_threshold_ = 20;
  const int min_fast_threshold_ = 7;
  const int patch_size_ = 31;
...
#endif  // WF_FEATURE_EXTRACTOR_FEATURE_EXTRACTOR_H
